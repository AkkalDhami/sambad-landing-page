...
export function FrequencyToggle({
  ...
  return (
    <div
      className={cn(
        "mx-auto flex w-fit rounded-full border bg-card p-2 shadow-xs",
        props.className
      )}
      {...props}
    >
      {frequencies.map((freq) => (
        <button
          className="relative rounded-full px-6 py-2 text-sm capitalize"
          key={freq}
          onClick={() => setFrequency(freq)}
          type="button"
        >
          <span className="relative z-10 font-medium">{freq}</span>
          {frequency === freq && (
            <motion.span
